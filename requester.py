...
def api_requester(args):
    ...
    save_path = os.path.join('./data/station_data/', file_name).replace('\\', '/')
    if os.path.isfile(save_path):
        print("이미 존재")
        return 
    
    url = "https://stcis.go.kr/pivotIndi/indiRgrstyExcelExport.do"
    headers = {'Cookie': 'vw0XJWB9NkpIf0GQOngJ3KnpboejH61kJ3VcohXbo9M6IJ9wzH4JbuyU9BEdww14.amV1c19kb21haW4vc2VydmVyMQ=='}
    payload = {
        "indiCd": "Z01710",
...
if __name__ == '__main__':
    ...
    station_req = open(os.path.join(data_dir, 'station_req.json').replace('\\', '/'), 'rb')
